...
Confirmado releyendo el código (no solo la palabra del audit anterior) que los puntos **5 y 6** de "Bugs confirmados" siguen sin arreglar, sin cambios desde el audit original:
- Punto 5 (`POST /cotizaciones` no valida que `tareaId` pertenezca a `proyectoId`): reproducido con `curl`/API directa (201 en vez de un rechazo). Sigue sin ser explotable por la UI real — `ModalCotizacion.tsx` del frontend solo ofrece tareas ya scopeadas al proyecto actual, así que ningún usuario normal puede armar esta combinación sin llamar la API a mano.
- Punto 6 (`OrdenCompraEstadoCambiadoOyente.resolverEmails`/`buscarUsuariosPorTipo` sin `try/catch` propio): confirmado en el código actual (`src/notificaciones/oyentes/orden-compra-estado-cambiado.oyente.ts`), sin cambios.
