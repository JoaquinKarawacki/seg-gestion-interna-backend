...
**Gotcha de entorno para `railway ssh keys add` en Windows con usuario con tilde** (ej. `Joaquín`): pasar la ruta de la clave pública con `-k <ruta>` falla con `Key not found` por un problema de encoding entre git-bash (MSYS) y el binario nativo de Railway al pasar caracteres acentuados como argumento. Solución: correr `railway ssh keys add` **sin argumentos** (auto-detección, sí encuentra la clave en `~/.ssh/` correctamente por dentro). Aparte, la primera conexión de `railway ssh --service <nombre>` puede fallar con `Host key verification failed` si se corre en un contexto no interactivo (sin TTY para confirmar el host key la primera vez) — no diagnosticado a fondo todavía, alternativa que funcionó: aplicar la migración desde la consola/shell web de Railway (dashboard) en vez de la CLI local.
