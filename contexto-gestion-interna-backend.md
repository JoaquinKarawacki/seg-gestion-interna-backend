...
**Corrección 2 (2026-08-20):** la "Cotización GENERAL" (`tareaId = null`) que describía este diagrama antes ya no existe — se reemplazó por una entidad separada, `PropuestaInversion`. `Cotizacion.tareaId` es obligatorio desde entonces: toda Cotización pertenece a una Tarea.

**Corrección 3 (2026-09-11):** `PropuestaInversion` (agregada en la Corrección 2) **se eliminó por completo** — ver detalle en "Avance de pago por proyecto" más abajo. El diagrama vigente ya no la incluye. `Cotizacion.tareaId` sigue siendo obligatorio (eso no volvió atrás), pero ya no existe ninguna entidad a nivel proyecto — la única fuente de costo/ejecución son las Cotizaciones de tarea y las OC.
  ...
  └── Proyecto (uno o varios)
        └── Tarea (una o varias, ej. "Electricidad", "Construcción")
              └── Cotización DE LA TAREA (una ACTIVA a la vez, versionada
                    ...
                    └── Monto total + moneda + ¿incluye IVA?
...
Las Cotizaciones de cada Tarea del mismo proyecto pueden estar todas `ACTIVA` al mismo tiempo — el versionado (marcar `REEMPLAZADA`) es independiente por cada Tarea.

**Consecuencia aceptada explícitamente (2026-08-20, sigue vigente tras la Corrección 3):** como `Cotizacion.tareaId` es obligatorio, ya no existe el caso "OC a nivel proyecto sin tarea" — toda OC vinculada a una `Cotizacion` (vía `cotizacionId`) va a derivar siempre un `tareaId`. Una OC sigue pudiendo existir "suelta" (sin `cotizacionId`, sin Cliente/Proyecto/Tarea).
...
- ~~`Proyecto.costoSegManual` (`Decimal?`) — override manual de "costo SEG"...~~ — **eliminado el 2026-09-11**, ver más abajo.
- ~~**Actualización 2026-08-20**: ya no es "una cotización general" la fuente de la moneda de referencia...~~ — **superado por la Actualización 2026-09-11**, ver más abajo.
...
**Actualización 2026-08-20 (cambio de modelo: Propuesta de Inversión reemplaza a la cotización general) — histórico, revertido el 2026-09-11, ver "Actualización 2026-09-11" más abajo.** El usuario pidió separar la propuesta comercial que se le pasa al cliente (a nivel de todo el proyecto) de las cotizaciones de proveedores por tarea, que hasta ahora vivían mezcladas en la misma entidad `Cotizacion` (la "cotización general", `tareaId = null`, con su campo `honorarios` opcional). Cambios de backend en su momento:
...
**Actualización 2026-09-11 (`PropuestaInversion` eliminada, IVA explícito en `Cotizacion`) — modelo vigente.** El usuario decidió sacar la Propuesta de Inversión por completo: la nueva tarjeta de costos del frontend calcula todo a partir de Cotizaciones de tarea y OC pagadas, sin necesitar una entidad comercial separada a nivel proyecto. Cambios reales (commit `b417138`, migración `20260911174559_eliminar_propuesta_inversion_y_agregar_iva_cotizacion`):
- Se borró `src/propuestas-inversion/` entero (controller, service, repositorio, DTOs, interfaz) y `PropuestasInversionModulo` de `app.module.ts`.
- `DROP TABLE propuestas_inversion` (con su FK a `proyectos`) y `Proyecto.costoSegManual` eliminado — ya no existe override manual de costo SEG ni el endpoint `POST /proyectos/:id/recalcular-costo-seg`.
- `Cotizacion` gana `ivaIncluido: Boolean @default(false)` (columna `iva_incluido`) — reemplaza al viejo `honorarios` como el dato que faltaba para calcular montos netos. `CrearCotizacionDto` lo acepta, `RespuestaCotizacionDto` lo devuelve.
- `ACCIONES_AUDITORIA` perdió `CREAR_PROPUESTA_INVERSION` (y las acciones de actualizar/eliminar propuesta si las había).
- **Cálculo de costo/ejecución sigue siendo 100% client-side** (no volvió el endpoint `avance-pago`, sigue la misma decisión de no implementarlo server-side) — ver `lib/proyectos/presentacion.ts` en `contexto-gestion-interna-frontend.md` para la fórmula vigente (neto de IVA, `TASA_IVA_URUGUAY = 0.22` fija).
- **Gotcha de despliegue real de este mismo cambio (2026-09-11 al 2026-09-14)**: esta migración quedó sin aplicar a mano en Railway durante 3 días — mismo patrón que el incidente del 21/08 descrito en "Comandos de Railway" más abajo. Esta vez se resolvió **sin** reactivar `prisma migrate deploy` en el `startCommand`: `railway ssh --service backend` seguido de `npx prisma migrate deploy` **dentro del contenedor ya corriendo** (usa el `DATABASE_URL` interno que el servicio ya tiene configurado, sin exponer nada públicamente ni arriesgar el proceso de arranque). Confirmado funcionando contra prod real. **Este es el método recomendado de acá en adelante** para aplicar migraciones pendientes, en vez de la alternativa de TCP proxy + `DATABASE_PUBLIC_URL` que sugería este documento (ese proyecto de Railway ni siquiera expone una `DATABASE_PUBLIC_URL`, ver más abajo).
...
**`prisma migrate deploy` NO corre automáticamente en el boot** (a diferencia de lo que decía antes este documento) — contra el Postgres real de Railway (template `postgres-ssl`), algo en la cadena `npx prisma migrate deploy && npm run start:prod` impide que el server llegue a levantar. Se sacó del `startCommand`. **Cualquier migración nueva de Prisma hay que aplicarla a mano** contra producción.

**Método recomendado (verificado 2026-09-14, ver incidente de `PropuestaInversion` parte 2 abajo)**: `railway ssh --service backend` para entrar al contenedor ya corriendo, y ahí dentro `npx prisma migrate deploy` — usa el `DATABASE_URL` interno que el servicio backend ya tiene seteado, sin exponer la base públicamente ni tocar el `startCommand`. El Postgres de este proyecto (template `postgres-ssl`) **no expone una variable `DATABASE_PUBLIC_URL`** (confirmado con `railway variables --service Postgres`, solo trae `DATABASE_URL`/`PGHOST`/`PGPORT`/etc. internos) — la alternativa de "TCP proxy temporal apuntando a `DATABASE_PUBLIC_URL`" que sugería antes este documento no aplica tal cual a este proyecto salvo que se cree un TCP proxy público a mano (`railway tcp-proxy`) primero. La otra alternativa que sigue existiendo pero es la más riesgosa es reagregar `prisma migrate deploy &&` al `startCommand` por un solo deploy y revertir después — **no usar salvo último recurso**, ver el incidente real más abajo.

**Gotcha de entorno para `railway ssh keys add` en Windows con usuario con tilde** (ej. `Joaquín`): pasar la ruta de la clave pública con `-k <ruta>` falla con `Key not found` por un problema de encoding entre git-bash (MSYS) y el binario nativo de Railway al pasar caracteres acentuados como argumento. Solución: correr `railway ssh keys add` **sin argumentos** (auto-detección, sí encuentra la clave en `~/.ssh/` correctamente por dentro).

**`Host key verification failed` en `railway ssh` no interactivo — resuelto (2026-09-16).** La primera conexión SSH a un service de Railway desde un contexto sin TTY (ej. corrida por un agente) fallaba porque no hay forma de confirmar interactivamente el host key la primera vez. Se resuelve pre-aceptándolo a mano antes de la primera conexión: `railway ssh config --service <nombre> --dry-run` muestra el `HostName` real (`ssh.railway.com` en este proyecto), y `ssh-keyscan -H ssh.railway.com >> ~/.ssh/known_hosts` lo agrega a `known_hosts` sin prompt. Después de eso, `railway ssh --service <nombre> -- <comando>` (modo no interactivo, pasando el comando como argumento en vez de abrir una shell) funciona normal. Ya no hace falta el rodeo de la consola web de Railway para esto.

**Incidente real (2026-08-21) que confirma este riesgo y agrega evidencia nueva**: la migración de `PropuestaInversion` (creada el 2026-08-20, ver `20260820135933_agregar_propuesta_inversion_y_tarea_obligatoria`) nunca se había aplicado a mano a producción — quedó afuera del proceso manual que sí se había seguido para las migraciones anteriores (sector opcional, costo SEG, tipo de cambio). Resultado: `GET /proyectos/:id/propuestas-inversion` y cualquier pantalla que lo usara devolvían `500 ERROR_INTERNO` en producción (la tabla `propuestas_inversion` no existía — confirmado en los logs de Railway: Prisma `P2021`, `relation "public.propuestas_inversion" does not exist`). Reportado por el usuario como "en propuesta de inversión aparece un error inesperado".

Al reagregar `prisma migrate deploy &&` al `startCommand` para aplicar esa migración (el procedimiento que este mismo documento sugiere arriba), la migración se aplicó correctamente (confirmado en logs: "The following migration(s) have been applied") **pero el proceso murió en silencio justo después, sin llegar a imprimir ningún log de `NestFactory`** — mismo síntoma que describe esta sección, con un dato nuevo: no es que `migrate deploy` "quede colgado sin salir nunca", el comando **sí termina** (se ve su output completo) — el problema está *después*, en la transición hacia `npm run start:prod`, y el proceso termina con lo que Railway interpreta como salida exitosa (`status: Completed`, sin reintentos de `restartPolicyType: ON_FAILURE`, que solo dispara ante una salida no-cero). Producción quedó caída (502 en toda la API) hasta revertir el `startCommand` a solo `npm run start:prod`. La migración ya aplicada no se perdió al revertir. **La causa raíz de por qué la cadena `&&` mata el proceso sigue sin diagnosticarse** — queda como deuda para investigar con calma, sin usuarios activos, no en caliente contra producción.

**Incidente real, parte 2 (2026-09-11 al 2026-09-14) — mismo patrón, esta vez sin causar caída**: la migración `20260911174559_eliminar_propuesta_inversion_y_agregar_iva_cotizacion` (ver "Modelo de dominio" → "Avance de pago por proyecto") se pusheó a `main` el 11/09 y quedó sin aplicar a mano durante 3 días — producción devolvió `500 ERROR_INTERNO` en cualquier `GET .../cotizaciones` durante todo ese tiempo (confirmado con curl real: `relation`/columna `iva_incluido` no existía todavía en la base). Esta vez **no se tocó el `startCommand`** — se aplicó con `railway ssh --service backend` → `npx prisma migrate deploy` dentro del contenedor ya corriendo (ver método recomendado más arriba), sin ningún downtime adicional más allá de los 3 días que ya llevaba caído por la migración faltante. Confirmado con curl que producción volvió a `200` inmediatamente después.
...
8. ~~`GET /proyectos/:id/avance-pago`, descrito en presente en la sección "Modelo de dominio" de este documento, **nunca se implementó**~~ — **resuelto por otro camino (2026-08-20, rediseñado 2026-09-11)**: la necesidad de negocio (monto cotizado vs. pagado, avance) quedó cubierta por la tarjeta **"Costos y ejecución"** de `/proyectos/[id]` (frontend), que calcula todo 100% client-side en `lib/proyectos/presentacion.ts` (`calcularResumenCostos`/`calcularDesglosePorTarea`) reusando los listados de cotizaciones/OC que la pantalla ya trae, sin este endpoint (la tarjeta se llamó "Comprometido" y después "Costos y rentabilidad" en versiones anteriores — nombre y campos vigentes desde 2026-09-11 tras eliminar `PropuestaInversion`). Se decide **no implementarlo por ahora** — se deja esta nota como referencia si en el futuro hiciera falta el cálculo server-side (ej. otro cliente además de este frontend, o necesidad de mayor precisión/consistencia que un cálculo hecho en el navegador no puede garantizar).
...
## Etapa 10 — Seed de datos reales (en curso)

**10a) Catálogos reales (2026-09-14, commit `d13de8f`+siguientes).** Se importaron clientes/proveedores/sectores/usuarios reales desde un Excel del historial de SEG: `prisma/datos-reales/{clientes,proveedores,sectores,usuarios}.json` + `prisma/importar-datos-reales.ts` (script standalone con `PrismaClient`+`PrismaPg`, `upsert` por `rut`/`email`, sin pasar por Nest/HTTP — no dispara eventos, notificaciones ni auditoría). Contraseña inicial de los usuarios nuevos: `Cambiar123!`. Se cargaron 503 clientes y 99 proveedores reales (con `banco`/`tipoCuenta`/`numeroCuenta` reales — **dato sensible en texto plano en el repo**, tenerlo presente si el repo cambia de visibilidad). `prisma/limpiar-datos-demo.ts` (mismo día) borró el dataset de demo/QA que había hasta entonces (proyectos "Edificio Torres del Puerto"/"Golf"/"Puerto", cliente "Constructora Bulevar SRL", 2 proveedores de prueba) — identificado por IDs fijos a propósito, no por nombre, para no arriesgar borrar datos reales por coincidencia.

**10b) Primer lote de datos transaccionales reales (2026-09-16).** Hasta este punto no existía ningún `Proyecto`/`Tarea`/`Cotizacion`/`OrdenCompra` real en la base. Se cargó el primer lote desde `Resumen_Proyectos_Osmosis_y_Bombas_de_Calor.xlsx` (carpeta padre `gestion-interna-seg/`, no versionado): 7 Proyectos (Osmosis×{CAMDEL,SOFITEL,UAM}, Bombas de Calor×{BSE,CAMDEL,COMTA,SOFITEL}), 14 Tareas (una por ítem de compra/Ref del Excel — nunca se agrupó más de un Ref bajo la misma Tarea, porque solo puede haber una Cotización `ACTIVA` por Tarea a la vez), 14 Cotizaciones (una por Tarea, `ivaIncluido: true`) y 30 OrdenCompra creadas **directamente en estado `PAGADO`** (una por pago real registrado, con su fila de `HistorialEstadoOC` fechada con la fecha real del pago) — no se simuló el flujo completo enviar→aprobar→confirmar-pago, es carga histórica. Total cargado: `2.330.542,52`, verificado exacto contra la hoja "Detalle de Pagos" del Excel antes de escribir nada.

Detalle relevante para no repetir el análisis:
- **La hoja "Resumen" del Excel tiene datos desactualizados** (Pagado/Saldo de CAMDEL y COMTA de "Bombas de Calor" no coinciden con la hoja de detalle ni con "Detalle de Pagos", que sí cuadran entre sí al centavo) — **no se usó como fuente de montos**, solo su columna Moneda (consistente) para saber qué iba en UYU vs. USD.
- **SOFITEL y UAM son clientes nuevos** que no existían en los datos reales de 10a — se crearon con RUT provisorio (`PENDIENTE-SOFITEL`/`PENDIENTE-UAM`, decisión del usuario) porque el Excel no trae RUT y es campo obligatorio/único. **Pendiente: reemplazar por el RUT real cuando se consiga.**
- Decisiones tomadas con el usuario para completar campos que el Excel no trae: `sectorId` = **AE** en Proyecto y en cada OrdenCompra; `solicitanteId` = usuario ADMIN (Joaquín Karawacki) en todas las OC; `formaPago` = `TRANSFERENCIA_BANCARIA` (asunción, no viene en el Excel, es solo descriptivo y no afecta ningún cálculo).
- `OS-SOFITEL-2` ("NAPA SOFITEL") se modeló como una segunda Tarea dentro del Proyecto "Osmosis" de SOFITEL (no como proyecto aparte), siguiendo el agrupamiento real de la hoja de detalle en vez de la fila suelta "NAPA" de la hoja Resumen.
- Tareas/Cotizaciones sin ningún pago registrado (`BC-CAMDEL-2`, nota de crédito de CIR todavía no emitida) **no generan ninguna OrdenCompra** — el saldo pendiente queda reflejado solo por la diferencia entre `Cotizacion.montoTotal` y la suma de OC, que es exactamente cómo la tarjeta "Costos y ejecución" del frontend ya calcula Costo SEG vs. Ejecución.
- Como `ivaIncluido: true` en todo lo cargado, la tarjeta "Costos y ejecución" va a mostrar estos montos **netos de IVA** (÷1.22), no los totales "IVA inc." tal cual figuran en el Excel — comportamiento normal ya vigente de `calcularResumenCostos`, no un bug de la carga.
- Scripts nuevos: `prisma/datos-reales/proyectos-osmosis-bombas-calor.json` (dataset ya transformado y validado) + `prisma/importar-proyectos-osmosis-bombas-calor.ts` (mismo patrón que 10a: resuelve clientes/proveedores por `rut`, guards de idempotencia por nombre antes de crear Proyecto/Tarea/Cotización, cada OC+historial en su propia `$transaction`) + `prisma/verificar-importacion-osmosis-bombas-calor.ts` (solo lectura, recalcula y compara contra el dataset; escrito pero **todavía no commiteado/corrido**, queda disponible si hace falta re-chequear algo puntual).
- Corrido directo contra producción vía `railway ssh --service backend -- npx tsx prisma/importar-proyectos-osmosis-bombas-calor.ts` (sin pasar primero por local — Docker Desktop no estaba levantado en esa sesión). Confirmado visualmente por el usuario en el sistema real.
- **Pendientes explícitos**: RUT real de SOFITEL/UAM; confirmar si CIR ya emitió la Nota de Crédito de `BC-CAMDEL-2` (para entonces sí cargar su OC de pago); número de factura real de `BC-SOFITEL-2` (figura como "***" en el Excel original).

---

## Un ENCARGADO puede aprobar más de un sector (2026-09-22, commit `c4cc5b7`)

**Esta sección reemplaza** las menciones de `usuario.sectorId === orden.sectorId` documentadas más arriba para Etapa 6 (líneas ~551, ~812, ~818), Etapa 7 (~825) y la auditoría QA (~867, ~900) — quedaron desactualizadas por este cambio, no se reescribieron línea por línea para no perder el historial de esas decisiones.

**Motivo:** se creó el sector "SEG EMOVE" y el usuario pidió que sus OC las apruebe `baccino@segingenieria.com`, que ya era `ENCARGADO` de VENTAS. El modelo hasta acá (`Usuario.sectorId`, uno a uno) no lo permitía sin hacerle perder VENTAS.

**Cambio de modelo:** se agregó `Usuario.sectoresEncargado ↔ Sector.encargados` (muchos a muchos, tabla implícita `_SectorEncargados`, `A`=sector/`B`=usuario). **`sectorId` no se tocó** — sigue siendo el sector de perfil/valor por defecto (formularios, filtros). `sectoresEncargado` es ahora la **única fuente de verdad** para "¿qué sectores puede aprobar/gestionar este usuario?", usada en los 4 lugares que antes comparaban `usuario.sectorId === orden.sectorId`:
- `ordenes-compra/aprobacion/ordenes-compra-aprobacion.service.ts` (`validarEncargadoDelSector`, aprobar/rechazar/anular)
- `comentarios/comentarios.service.ts` (`esEncargadoDelSector`, dispara la transición automática a `EN_CONSULTA`)
- `ordenes-compra/ordenes-compra.service.ts` (`validarPertenencia`, ver/editar una OC ajena)
- `usuarios/usuarios.repositorio.ts` (`buscarActivosPorRol`, a quién notificar por mail como `ENCARGADO_SECTOR`)

La migración (`20260922191857_agregar_sectores_encargado`) hace backfill: todo `ENCARGADO` existente quedó con su `sectorId` de entonces ya cargado en `sectoresEncargado`, así nadie perdió permisos. Aplicada en producción **antes** de que terminara de desplegar el código nuevo (vía `railway ssh --service backend -- npx prisma migrate deploy`, service todavía sirviendo la versión vieja) — evita la ventana en la que el código nuevo consultaría una tabla que todavía no existe.

**JWT:** viaja `sectoresEncargado: string[]` junto a `sectorId`, igual patrón (`jwt.estrategia.ts` hace `payload.sectoresEncargado ?? []` por si quedara algún token viejo sin el campo — no rompe con tokens emitidos antes de este cambio).

**API de usuarios:** `POST/PATCH /usuarios` acepta `sectoresEncargadoIds: string[]` (reemplaza la lista completa en `PATCH`, no hace merge). `GET /ordenes-compra?sectorId=` ahora también acepta una lista separada por comas (`{ in: [...] }` en Prisma) — lo usa el dashboard del frontend para contar "pendientes de mi aprobación" sumando todos los sectores del encargado, no solo uno.

**Pendiente**: asignar "SEG EMOVE" a Baccino quedó para hacerse desde la UI (Usuarios → editar → tildar el sector nuevo en "Sectores que puede aprobar"), no se scripteó.
